...
def test_workflow_help_includes_metadata_and_review_index_steps() -> None:
    ...
    assert "pubtator_ground_question" in names
    assert "pubtator_search_biomedical_entities" in names
    assert "pubtator_find_entity_relations" in names
    assert "pubtator_search_literature" in names
    assert "pubtator_get_publication_metadata" in names
    assert "pubtator_index_review_evidence" in names
    assert "pubtator_retrieve_review_context_batch" in names
...
def test_workflow_help_entity_discovery_uses_discovery_tools() -> None:
    ...
    assert response.meta["next_commands"][0] == "pubtator_search_biomedical_entities"
...
def test_workflow_help_mentions_recovery_quote_confidence_and_audit_trail() -> None:
    ...
    assert "pubtator_get_review_audit_trail" in help_text
...
def test_workflow_help_documents_guideline_search_as_filtered_literature_search() -> None:
    ...
    assert "pubtator_search_guidelines" in payload
...
def test_workflow_help_routes_hyphenated_literature_graph_aliases() -> None:
    for task in ("topic-map", "topic map", "literature-map", "literature map", "citation graph"):
        ...
        assert "pubtator_build_topic_literature_map" in response.tool_sequence
        assert "pubtator_get_publication_citation_graph" in response.tool_sequence
...
def test_workflow_help_keeps_citation_audit_aliases_on_audit_workflow() -> None:
    for task in (
        ...
        assert response.tool_sequence[0] == "pubtator_lookup_citation"
        assert "pubtator_get_publication_citation_graph" not in response.tool_sequence
