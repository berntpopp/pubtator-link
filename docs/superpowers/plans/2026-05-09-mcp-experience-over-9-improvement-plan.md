...
- `preflight_review_sources` errors include an actionable fallback command to `get_publication_passages`.
...
- Modify: `pubtator_link/mcp/tools/review.py`
  - Pass explicit fallback metadata into `run_mcp_tool` for `preflight_review_sources`.
...
def _recovery_text_for_context(context: McpErrorContext, fallback_tool: str | None) -> str:
    ...
    return (
        "Run diagnostics. If the review schema is stale, apply database migrations "
        "and retry."
    )
```
