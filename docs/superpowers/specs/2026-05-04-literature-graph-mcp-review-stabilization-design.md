...
For source paper metadata, PMCID implies `has_pmc_full_text=true`, but it does
not by itself imply reusable open access. `is_open_access=true` should come from
explicit OA evidence such as Europe PMC `isOpenAccess`, OpenAlex
`open_access.is_oa`, Unpaywall `is_oa`, or a provider license field when one is
available. This keeps PMC full-text availability separate from rights and reuse
semantics.
...
Do not reintroduce `prepare_mode` into live MCP schemas. The current public
schema does not expose it, so compatibility work is limited to removing stale
generated examples, hints, and tests that still mention it.
...
This server can make the workflow more discoverable, but it cannot remove host
ToolSearch gating by itself. The implementation should document that boundary
instead of implying the server controls first-use tool loading.
...
- `prepare_mode` is not part of the live MCP schema and should not appear in
  active tool examples, generated next commands, or catalog guidance.

## Sequencing

Each phase should land as one or more self-contained commits with focused tests
green before the next phase changes behavior. Phase 1's resolver consistency
work should be implemented and exercised before Phase 2 changes compact response
contracts.

## Known Follow-Up

The review also noted `_meta` redundancy, especially duplicated
`provider_status` and `next_commands` blocks. This pass may reduce repetition
where it directly touches a response, but a broader response-envelope cleanup is
deferred so the resolver, compact-mode, and budget fixes stay reviewable.
