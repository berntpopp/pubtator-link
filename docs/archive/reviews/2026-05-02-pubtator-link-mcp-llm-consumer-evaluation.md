...
| R4 | **Add `pubtator.health` / `diagnostics`** exposing per-subsystem status (BioC fetcher, indexer DB, RAG store) so LLMs can route around outages instead of guessing. | 1/4 | M |
...
| D3 | **Add `publication_types` filter / guideline boost.** All 4 runs failed to surface the 2016 EULAR FMF recommendations even when explicitly named, and 3/4 picked a different EULAR-anchor paper as a result. Either honor `publication_types=Practice Guideline,Consensus` properly on legacy MEDLINE entries, or expose a `search_guidelines` shortcut. | 3/4 | M |
...
- `get_publication_metadata` for citation-grade PMID metadata.
...
- `workflow_help` for canonical workflow guidance.
- `suggest_corpus` for compact review-feeding PMID selection.
