...
| Review quickstart handoff | Shipped | `review_quickstart(topic, n_pmids=8)` searches, stages/indexes, inspects, and returns a retrieval handoff. |
...
**Why this matters:** when a Claude/Cursor user runs `get_review_context_batch` against degraded evidence, the host can surface a server-emitted notice in the same chat thread. This is now implemented for review evidence degraded modes.
...
@mcp.tool(name="get_review_context_batch", ...)
async def retrieve_review_context_batch(review_id: str, queries: list[str], ..., ctx: Context | None = None):
    if not await service.review_index_has_passages(review_id):
        await ctx.warning(
            "Review index has no prepared passages - run index_review_evidence first.",
        )
    ...
```
...
**Shipped:** `index_review_evidence`, `get_review_context`, and `get_review_context_batch` now accept FastMCP-injected `ctx` without exposing it in public JSON schema, and emit warnings for degraded review evidence.
...
@instrument_tool("search_literature")
...
| 10 | Add audit trail helper | Implemented as `get_review_audit_trail` |
